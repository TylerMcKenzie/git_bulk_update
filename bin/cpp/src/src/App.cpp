// Generated by Haxe 3.4.7
#include <hxcpp.h>

#ifndef INCLUDED_EReg
#include <EReg.h>
#endif
#ifndef INCLUDED_Sys
#include <Sys.h>
#endif
#ifndef INCLUDED_haxe_io_Bytes
#include <haxe/io/Bytes.h>
#endif
#ifndef INCLUDED_haxe_io_Input
#include <haxe/io/Input.h>
#endif
#ifndef INCLUDED_haxe_io_Path
#include <haxe/io/Path.h>
#endif
#ifndef INCLUDED_src_App
#include <src/App.h>
#endif
#ifndef INCLUDED_src_Cli
#include <src/Cli.h>
#endif
#ifndef INCLUDED_src_util_Hub
#include <src/util/Hub.h>
#endif
#ifndef INCLUDED_sys_FileSystem
#include <sys/FileSystem.h>
#endif
#ifndef INCLUDED_sys_io_File
#include <sys/io/File.h>
#endif
#ifndef INCLUDED_sys_io_Process
#include <sys/io/Process.h>
#endif

HX_DEFINE_STACK_FRAME(_hx_pos_9aa8d6444802cb78_12_new,"src.App","new",0x5207d669,"src.App.new","App.hx",12,0xc43e94dd)
HX_LOCAL_STACK_FRAME(_hx_pos_9aa8d6444802cb78_48_run,"src.App","run",0x520aed54,"src.App.run","App.hx",48,0xc43e94dd)
static const ::String _hx_array_data_62186df7_4[] = {
	HX_("diff",05,5c,69,42),HX_("--name-only",8e,ea,71,0d),
};
static const ::String _hx_array_data_62186df7_5[] = {
	HX_("diff",05,5c,69,42),
};
HX_LOCAL_STACK_FRAME(_hx_pos_9aa8d6444802cb78_92_run,"src.App","run",0x520aed54,"src.App.run","App.hx",92,0xc43e94dd)
static const ::String _hx_array_data_62186df7_7[] = {
	HX_("push",da,11,61,4a),HX_("-u",a8,27,00,00),
};
static const ::String _hx_array_data_62186df7_8[] = {
	HX_("checkout",c6,b4,ff,ac),HX_("-",2d,00,00,00),
};
HX_LOCAL_STACK_FRAME(_hx_pos_9aa8d6444802cb78_107_error,"src.App","error",0x9e722611,"src.App.error","App.hx",107,0xc43e94dd)
HX_LOCAL_STACK_FRAME(_hx_pos_9aa8d6444802cb78_113_getAllFiles,"src.App","getAllFiles",0x58171755,"src.App.getAllFiles","App.hx",113,0xc43e94dd)
HX_LOCAL_STACK_FRAME(_hx_pos_9aa8d6444802cb78_138_searchAndReplaceInFile,"src.App","searchAndReplaceInFile",0x3c42f21d,"src.App.searchAndReplaceInFile","App.hx",138,0xc43e94dd)
namespace src{

void App_obj::__construct(){
            	HX_STACKFRAME(&_hx_pos_9aa8d6444802cb78_12_new)
HXLINE(  36)		this->chunk = (int)50;
HXLINE(  43)		super::__construct();
HXLINE(  65)		{
HXLINE(  19)			::Array< ::String > args = ::Sys_obj::args();
HXLINE(  52)			::String _hx_tmp;
HXDLIN(  52)			if ((args->indexOf(HX_("-d",97,27,00,00),null()) > (int)-1)) {
HXLINE(  52)				_hx_tmp = args->__get((args->indexOf(HX_("-d",97,27,00,00),null()) + (int)1));
            			}
            			else {
HXLINE(  52)				_hx_tmp = null();
            			}
HXDLIN(  52)			this->directory = _hx_tmp;
HXDLIN(  52)			::String _hx_tmp1;
HXDLIN(  52)			if ((args->indexOf(HX_("-f",99,27,00,00),null()) > (int)-1)) {
HXLINE(  52)				_hx_tmp1 = args->__get((args->indexOf(HX_("-f",99,27,00,00),null()) + (int)1));
            			}
            			else {
HXLINE(  52)				_hx_tmp1 = null();
            			}
HXDLIN(  52)			this->fileExtension = _hx_tmp1;
HXDLIN(  52)			::String _hx_tmp2;
HXDLIN(  52)			if ((args->indexOf(HX_("-s",a6,27,00,00),null()) > (int)-1)) {
HXLINE(  52)				_hx_tmp2 = args->__get((args->indexOf(HX_("-s",a6,27,00,00),null()) + (int)1));
            			}
            			else {
HXLINE(  52)				_hx_tmp2 = null();
            			}
HXDLIN(  52)			this->search = _hx_tmp2;
HXDLIN(  52)			::String _hx_tmp3;
HXDLIN(  52)			if ((args->indexOf(HX_("-r",a5,27,00,00),null()) > (int)-1)) {
HXLINE(  52)				_hx_tmp3 = args->__get((args->indexOf(HX_("-r",a5,27,00,00),null()) + (int)1));
            			}
            			else {
HXLINE(  52)				_hx_tmp3 = null();
            			}
HXDLIN(  52)			this->replace = _hx_tmp3;
HXDLIN(  52)			::String _hx_tmp4;
HXDLIN(  52)			if ((args->indexOf(HX_("-b",95,27,00,00),null()) > (int)-1)) {
HXLINE(  52)				_hx_tmp4 = args->__get((args->indexOf(HX_("-b",95,27,00,00),null()) + (int)1));
            			}
            			else {
HXLINE(  52)				_hx_tmp4 = null();
            			}
HXDLIN(  52)			this->branchname = _hx_tmp4;
HXLINE(  42)			this->createPull = (args->indexOf(HX_("--create-pull",96,42,33,9b),null()) > (int)-1);
HXLINE(  52)			::String _hx_tmp5;
HXDLIN(  52)			if ((args->indexOf(HX_("--pull-message",5f,d5,e6,e9),null()) > (int)-1)) {
HXLINE(  52)				_hx_tmp5 = args->__get((args->indexOf(HX_("--pull-message",5f,d5,e6,e9),null()) + (int)1));
            			}
            			else {
HXLINE(  52)				_hx_tmp5 = null();
            			}
HXDLIN(  52)			this->pullRequestMessage = _hx_tmp5;
HXLINE(  42)			this->dryRun = (args->indexOf(HX_("--dry",cb,f9,12,07),null()) > (int)-1);
            		}
            	}

Dynamic App_obj::__CreateEmpty() { return new App_obj; }

void *App_obj::_hx_vtable = 0;

Dynamic App_obj::__Create(hx::DynamicArray inArgs)
{
	hx::ObjectPtr< App_obj > _hx_result = new App_obj();
	_hx_result->__construct();
	return _hx_result;
}

bool App_obj::_hx_isInstanceOf(int inClassId) {
	if (inClassId<=(int)0x5752985d) {
		return inClassId==(int)0x00000001 || inClassId==(int)0x5752985d;
	} else {
		return inClassId==(int)0x5754195c;
	}
}

void App_obj::run(){
            	HX_GC_STACKFRAME(&_hx_pos_9aa8d6444802cb78_48_run)
HXLINE(  49)		if (hx::IsNull( this->directory )) {
HXLINE(  50)			this->error(HX_("Test directory is required",f4,43,19,c2));
            		}
HXLINE(  53)		::String files = this->directory;
HXDLIN(  53)		::Array< ::String > files1 = this->getAllFiles(files,::Array_obj< ::String >::__new());
HXLINE(  55)		int range = (int)0;
HXLINE(  58)		{
HXLINE(  58)			int _g_index;
HXDLIN(  58)			int _g_chunkSize;
HXDLIN(  58)			::cpp::VirtualArray _g_array;
HXDLIN(  58)			_g_index = (int)0;
HXDLIN(  58)			_g_array = files1;
HXDLIN(  58)			_g_chunkSize = this->chunk;
HXDLIN(  58)			while((_g_index < _g_array->get_length())){
HXLINE(  58)				int _g_index1 = _g_index;
HXDLIN(  58)				_g_index = (_g_index + _g_chunkSize);
HXDLIN(  58)				::cpp::VirtualArray filesChunk = _g_array->slice(_g_index1,_g_index);
HXLINE(  59)				{
HXLINE(  59)					int _g = (int)0;
HXDLIN(  59)					while((_g < filesChunk->get_length())){
HXLINE(  59)						 ::Dynamic file = filesChunk->__get(_g);
HXDLIN(  59)						_g = (_g + (int)1);
HXLINE(  60)						this->searchAndReplaceInFile(this->search,this->replace,( (::String)(file) ));
            					}
            				}
HXLINE(  63)				if (this->dryRun) {
HXLINE(  64)					 ::sys::io::Process testProcess =  ::sys::io::Process_obj::__alloc( HX_CTX ,HX_("git",12,84,4e,00),::Array_obj< ::String >::fromData( _hx_array_data_62186df7_4,2));
HXLINE(  66)					if ((testProcess->_hx_stdout->readAll(null())->toString().length > (int)0)) {
HXLINE(  67)						::Sys_obj::command(HX_("git",12,84,4e,00),::Array_obj< ::String >::fromData( _hx_array_data_62186df7_5,1));
HXLINE(  68)						::Sys_obj::command(HX_("git",12,84,4e,00),::Array_obj< ::String >::__new(2)->init(0,HX_("checkout",c6,b4,ff,ac))->init(1,this->directory));
HXLINE(  69)						testProcess->close();
HXLINE(  70)						return;
            					}
HXLINE(  73)					testProcess->close();
            				}
HXLINE(  77)				bool _hx_tmp;
HXDLIN(  77)				if (this->createPull) {
HXLINE(  77)					_hx_tmp = !(this->dryRun);
            				}
            				else {
HXLINE(  77)					_hx_tmp = false;
            				}
HXDLIN(  77)				if (_hx_tmp) {
HXLINE(  78)					int start = range;
HXLINE(  79)					range = (range + this->chunk);
HXDLIN(  79)					int end = range;
HXLINE(  81)					if (hx::IsNull( this->branchname )) {
HXLINE(  81)						this->error(HX_("'-b' branch flag is required when creating a pull request.",84,6d,a7,d3));
            					}
HXLINE(  84)					::String branchnameRange = ((((this->branchname + HX_("_batch_",04,ed,46,c4)) + start) + HX_("_",5f,00,00,00)) + end);
HXLINE(  86)					if (hx::IsEq(  ::sys::io::Process_obj::__alloc( HX_CTX ,HX_("git",12,84,4e,00),::Array_obj< ::String >::__new(4)->init(0,HX_("checkout",c6,b4,ff,ac))->init(1,HX_("-b",95,27,00,00))->init(2,branchnameRange)->init(3,HX_("master",a2,80,20,bb)))->exitCode(null()),(int)0 )) {
            						HX_BEGIN_LOCAL_FUNC_S0(hx::LocalFunc,_hx_Closure_0) HXARGC(1)
            						void _hx_run( ::sys::io::Process process){
            							HX_GC_STACKFRAME(&_hx_pos_9aa8d6444802cb78_92_run)
HXLINE(  93)							::Sys_obj::print(process->_hx_stdout->readAll(null())->toString());
HXLINE(  94)							process->exitCode(null());
HXLINE(  95)							process->close();
            						}
            						HX_END_LOCAL_FUNC1((void))

HXLINE(  87)						 ::sys::io::Process_obj::__alloc( HX_CTX ,HX_("git",12,84,4e,00),::Array_obj< ::String >::__new(3)->init(0,HX_("commit",f7,6f,2e,c7))->init(1,HX_("-am",59,7a,22,00))->init(2,(((HX_("Adding update for batch ",b5,75,7f,e2) + start) + HX_(" - ",73,6f,18,00)) + end)))->exitCode(null());
HXLINE(  88)						 ::sys::io::Process_obj::__alloc( HX_CTX ,HX_("git",12,84,4e,00),::Array_obj< ::String >::fromData( _hx_array_data_62186df7_7,2))->exitCode(null());
HXLINE(  90)						::String message;
HXDLIN(  90)						if (hx::IsNotNull( this->pullRequestMessage )) {
HXLINE(  90)							message = this->pullRequestMessage;
            						}
            						else {
HXLINE(  90)							message = HX_("",00,00,00,00);
            						}
HXLINE(  92)						::src::util::Hub_obj::pullRequest(::Array_obj< ::String >::__new(4)->init(0,HX_("-m",a0,27,00,00))->init(1,(((HX_("Update batch ",3d,40,8c,ce) + start) + HX_(" - ",73,6f,18,00)) + end))->init(2,HX_("-m",a0,27,00,00))->init(3,message), ::Dynamic(new _hx_Closure_0()));
HXLINE(  98)						 ::sys::io::Process_obj::__alloc( HX_CTX ,HX_("git",12,84,4e,00),::Array_obj< ::String >::fromData( _hx_array_data_62186df7_8,2))->exitCode(null());
            					}
            					else {
HXLINE( 100)						this->error(((HX_("Could not checkout branch '",f7,8e,c5,dc) + branchnameRange) + HX_("'.",27,22,00,00)));
            					}
            				}
            			}
            		}
            	}


void App_obj::error(::String __o_msg){
::String msg = __o_msg.Default(HX_HCSTRING("","\x00","\x00","\x00","\x00"));
            	HX_STACKFRAME(&_hx_pos_9aa8d6444802cb78_107_error)
HXLINE( 108)		::Sys_obj::println(msg);
HXLINE( 109)		::Sys_obj::exit((int)1);
            	}


HX_DEFINE_DYNAMIC_FUNC1(App_obj,error,(void))

::Array< ::String > App_obj::getAllFiles(::String directory,::Array< ::String > files){
            	HX_STACKFRAME(&_hx_pos_9aa8d6444802cb78_113_getAllFiles)
HXLINE( 114)		if (::sys::FileSystem_obj::exists(directory)) {
HXLINE( 115)			if (::sys::FileSystem_obj::isDirectory(directory)) {
HXLINE( 116)				int _g = (int)0;
HXDLIN( 116)				::Array< ::String > _g1 = ::sys::FileSystem_obj::readDirectory(directory);
HXDLIN( 116)				while((_g < _g1->length)){
HXLINE( 116)					::String file = _g1->__get(_g);
HXDLIN( 116)					_g = (_g + (int)1);
HXLINE( 117)					::String filePath = ::haxe::io::Path_obj::join(::Array_obj< ::String >::__new(2)->init(0,directory)->init(1,file));
HXLINE( 119)					if (!(::sys::FileSystem_obj::isDirectory(filePath))) {
HXLINE( 120)						::String _hx_tmp = ::haxe::io::Path_obj::extension(filePath);
HXDLIN( 120)						if ((_hx_tmp == this->fileExtension)) {
HXLINE( 121)							files->push(filePath);
            						}
            					}
            					else {
HXLINE( 124)						this->getAllFiles(::haxe::io::Path_obj::addTrailingSlash(filePath),files);
            					}
            				}
            			}
            			else {
HXLINE( 128)				files->push(directory);
            			}
            		}
            		else {
HXLINE( 131)			this->error(((HX_("Directory '",94,db,6a,45) + directory) + HX_("' does not exist",9c,d9,8e,aa)));
            		}
HXLINE( 134)		return files;
            	}


HX_DEFINE_DYNAMIC_FUNC2(App_obj,getAllFiles,return )

void App_obj::searchAndReplaceInFile(::String __o_search,::String __o_replace,::String filePath){
::String search = __o_search.Default(HX_HCSTRING("","\x00","\x00","\x00","\x00"));
::String replace = __o_replace.Default(HX_HCSTRING("","\x00","\x00","\x00","\x00"));
            	HX_GC_STACKFRAME(&_hx_pos_9aa8d6444802cb78_138_searchAndReplaceInFile)
HXLINE( 139)		if (!(::sys::FileSystem_obj::exists(filePath))) {
HXLINE( 140)			this->error(((HX_("File: '",a5,ad,8c,cc) + filePath) + HX_("' does not exist",9c,d9,8e,aa)));
            		}
HXLINE( 144)		bool _hx_tmp;
HXDLIN( 144)		if ((search.length != (int)0)) {
HXLINE( 144)			_hx_tmp = (replace.length == (int)0);
            		}
            		else {
HXLINE( 144)			_hx_tmp = true;
            		}
HXLINE( 143)		if (_hx_tmp) {
HXLINE( 147)			this->error(HX_("Search and Replace are required",b8,23,66,c5));
            		}
HXLINE( 150)		::String fileContent = ::sys::io::File_obj::getContent(filePath);
HXLINE( 151)		 ::EReg searchRegex =  ::EReg_obj::__alloc( HX_CTX ,search,HX_("g",67,00,00,00));
HXLINE( 152)		 ::EReg newLineRegex =  ::EReg_obj::__alloc( HX_CTX ,HX_("\\\\n",ee,1f,46,00),HX_("g",67,00,00,00));
HXLINE( 154)		::String updatedContent = searchRegex->replace(fileContent,newLineRegex->replace(replace,HX_("\n",0a,00,00,00)));
HXLINE( 156)		try {
            			HX_STACK_CATCHABLE(::String, 0);
HXLINE( 157)			::sys::io::File_obj::saveContent(filePath,updatedContent);
            		}
            		catch( ::Dynamic _hx_e){
            			if (_hx_e.IsClass< ::String >() ){
            				HX_STACK_BEGIN_CATCH
            				::String errorMessage = _hx_e;
HXLINE( 159)				this->error(errorMessage);
            			}
            			else {
            				HX_STACK_DO_THROW(_hx_e);
            			}
            		}
            	}


HX_DEFINE_DYNAMIC_FUNC3(App_obj,searchAndReplaceInFile,(void))


hx::ObjectPtr< App_obj > App_obj::__new() {
	hx::ObjectPtr< App_obj > __this = new App_obj();
	__this->__construct();
	return __this;
}

hx::ObjectPtr< App_obj > App_obj::__alloc(hx::Ctx *_hx_ctx) {
	App_obj *__this = (App_obj*)(hx::Ctx::alloc(_hx_ctx, sizeof(App_obj), true, "src.App"));
	*(void **)__this = App_obj::_hx_vtable;
	__this->__construct();
	return __this;
}

App_obj::App_obj()
{
}

void App_obj::__Mark(HX_MARK_PARAMS)
{
	HX_MARK_BEGIN_CLASS(App);
	HX_MARK_MEMBER_NAME(directory,"directory");
	HX_MARK_MEMBER_NAME(fileExtension,"fileExtension");
	HX_MARK_MEMBER_NAME(search,"search");
	HX_MARK_MEMBER_NAME(replace,"replace");
	HX_MARK_MEMBER_NAME(branchname,"branchname");
	HX_MARK_MEMBER_NAME(createPull,"createPull");
	HX_MARK_MEMBER_NAME(pullRequestMessage,"pullRequestMessage");
	HX_MARK_MEMBER_NAME(chunk,"chunk");
	HX_MARK_MEMBER_NAME(dryRun,"dryRun");
	HX_MARK_END_CLASS();
}

void App_obj::__Visit(HX_VISIT_PARAMS)
{
	HX_VISIT_MEMBER_NAME(directory,"directory");
	HX_VISIT_MEMBER_NAME(fileExtension,"fileExtension");
	HX_VISIT_MEMBER_NAME(search,"search");
	HX_VISIT_MEMBER_NAME(replace,"replace");
	HX_VISIT_MEMBER_NAME(branchname,"branchname");
	HX_VISIT_MEMBER_NAME(createPull,"createPull");
	HX_VISIT_MEMBER_NAME(pullRequestMessage,"pullRequestMessage");
	HX_VISIT_MEMBER_NAME(chunk,"chunk");
	HX_VISIT_MEMBER_NAME(dryRun,"dryRun");
}

hx::Val App_obj::__Field(const ::String &inName,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 3:
		if (HX_FIELD_EQ(inName,"run") ) { return hx::Val( run_dyn() ); }
		break;
	case 5:
		if (HX_FIELD_EQ(inName,"chunk") ) { return hx::Val( chunk ); }
		if (HX_FIELD_EQ(inName,"error") ) { return hx::Val( error_dyn() ); }
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"search") ) { return hx::Val( search ); }
		if (HX_FIELD_EQ(inName,"dryRun") ) { return hx::Val( dryRun ); }
		break;
	case 7:
		if (HX_FIELD_EQ(inName,"replace") ) { return hx::Val( replace ); }
		break;
	case 9:
		if (HX_FIELD_EQ(inName,"directory") ) { return hx::Val( directory ); }
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"branchname") ) { return hx::Val( branchname ); }
		if (HX_FIELD_EQ(inName,"createPull") ) { return hx::Val( createPull ); }
		break;
	case 11:
		if (HX_FIELD_EQ(inName,"getAllFiles") ) { return hx::Val( getAllFiles_dyn() ); }
		break;
	case 13:
		if (HX_FIELD_EQ(inName,"fileExtension") ) { return hx::Val( fileExtension ); }
		break;
	case 18:
		if (HX_FIELD_EQ(inName,"pullRequestMessage") ) { return hx::Val( pullRequestMessage ); }
		break;
	case 22:
		if (HX_FIELD_EQ(inName,"searchAndReplaceInFile") ) { return hx::Val( searchAndReplaceInFile_dyn() ); }
	}
	return super::__Field(inName,inCallProp);
}

hx::Val App_obj::__SetField(const ::String &inName,const hx::Val &inValue,hx::PropertyAccess inCallProp)
{
	switch(inName.length) {
	case 5:
		if (HX_FIELD_EQ(inName,"chunk") ) { chunk=inValue.Cast<  ::Dynamic >(); return inValue; }
		break;
	case 6:
		if (HX_FIELD_EQ(inName,"search") ) { search=inValue.Cast< ::String >(); return inValue; }
		if (HX_FIELD_EQ(inName,"dryRun") ) { dryRun=inValue.Cast< bool >(); return inValue; }
		break;
	case 7:
		if (HX_FIELD_EQ(inName,"replace") ) { replace=inValue.Cast< ::String >(); return inValue; }
		break;
	case 9:
		if (HX_FIELD_EQ(inName,"directory") ) { directory=inValue.Cast< ::String >(); return inValue; }
		break;
	case 10:
		if (HX_FIELD_EQ(inName,"branchname") ) { branchname=inValue.Cast< ::String >(); return inValue; }
		if (HX_FIELD_EQ(inName,"createPull") ) { createPull=inValue.Cast< bool >(); return inValue; }
		break;
	case 13:
		if (HX_FIELD_EQ(inName,"fileExtension") ) { fileExtension=inValue.Cast< ::String >(); return inValue; }
		break;
	case 18:
		if (HX_FIELD_EQ(inName,"pullRequestMessage") ) { pullRequestMessage=inValue.Cast< ::String >(); return inValue; }
	}
	return super::__SetField(inName,inValue,inCallProp);
}

void App_obj::__GetFields(Array< ::String> &outFields)
{
	outFields->push(HX_HCSTRING("directory","\x6d","\xf2","\x44","\x10"));
	outFields->push(HX_HCSTRING("fileExtension","\x63","\xf5","\x4f","\x84"));
	outFields->push(HX_HCSTRING("search","\x68","\x9f","\xf7","\x62"));
	outFields->push(HX_HCSTRING("replace","\x34","\x48","\x28","\xab"));
	outFields->push(HX_HCSTRING("branchname","\xed","\x69","\x59","\xd6"));
	outFields->push(HX_HCSTRING("createPull","\xe1","\xcc","\xee","\x5c"));
	outFields->push(HX_HCSTRING("pullRequestMessage","\xdd","\x64","\x4e","\x85"));
	outFields->push(HX_HCSTRING("chunk","\x6d","\xc6","\xc2","\x45"));
	outFields->push(HX_HCSTRING("dryRun","\xe0","\xa6","\x97","\xef"));
	super::__GetFields(outFields);
};

#if HXCPP_SCRIPTABLE
static hx::StorageInfo App_obj_sMemberStorageInfo[] = {
	{hx::fsString,(int)offsetof(App_obj,directory),HX_HCSTRING("directory","\x6d","\xf2","\x44","\x10")},
	{hx::fsString,(int)offsetof(App_obj,fileExtension),HX_HCSTRING("fileExtension","\x63","\xf5","\x4f","\x84")},
	{hx::fsString,(int)offsetof(App_obj,search),HX_HCSTRING("search","\x68","\x9f","\xf7","\x62")},
	{hx::fsString,(int)offsetof(App_obj,replace),HX_HCSTRING("replace","\x34","\x48","\x28","\xab")},
	{hx::fsString,(int)offsetof(App_obj,branchname),HX_HCSTRING("branchname","\xed","\x69","\x59","\xd6")},
	{hx::fsBool,(int)offsetof(App_obj,createPull),HX_HCSTRING("createPull","\xe1","\xcc","\xee","\x5c")},
	{hx::fsString,(int)offsetof(App_obj,pullRequestMessage),HX_HCSTRING("pullRequestMessage","\xdd","\x64","\x4e","\x85")},
	{hx::fsObject /*Dynamic*/ ,(int)offsetof(App_obj,chunk),HX_HCSTRING("chunk","\x6d","\xc6","\xc2","\x45")},
	{hx::fsBool,(int)offsetof(App_obj,dryRun),HX_HCSTRING("dryRun","\xe0","\xa6","\x97","\xef")},
	{ hx::fsUnknown, 0, null()}
};
static hx::StaticInfo *App_obj_sStaticStorageInfo = 0;
#endif

static ::String App_obj_sMemberFields[] = {
	HX_HCSTRING("directory","\x6d","\xf2","\x44","\x10"),
	HX_HCSTRING("fileExtension","\x63","\xf5","\x4f","\x84"),
	HX_HCSTRING("search","\x68","\x9f","\xf7","\x62"),
	HX_HCSTRING("replace","\x34","\x48","\x28","\xab"),
	HX_HCSTRING("branchname","\xed","\x69","\x59","\xd6"),
	HX_HCSTRING("createPull","\xe1","\xcc","\xee","\x5c"),
	HX_HCSTRING("pullRequestMessage","\xdd","\x64","\x4e","\x85"),
	HX_HCSTRING("chunk","\x6d","\xc6","\xc2","\x45"),
	HX_HCSTRING("dryRun","\xe0","\xa6","\x97","\xef"),
	HX_HCSTRING("run","\x4b","\xe7","\x56","\x00"),
	HX_HCSTRING("error","\xc8","\xcb","\x29","\x73"),
	HX_HCSTRING("getAllFiles","\x4c","\x60","\x70","\x1f"),
	HX_HCSTRING("searchAndReplaceInFile","\x86","\x6e","\x11","\x38"),
	::String(null()) };

static void App_obj_sMarkStatics(HX_MARK_PARAMS) {
	HX_MARK_MEMBER_NAME(App_obj::__mClass,"__mClass");
};

#ifdef HXCPP_VISIT_ALLOCS
static void App_obj_sVisitStatics(HX_VISIT_PARAMS) {
	HX_VISIT_MEMBER_NAME(App_obj::__mClass,"__mClass");
};

#endif

hx::Class App_obj::__mClass;

void App_obj::__register()
{
	hx::Object *dummy = new App_obj;
	App_obj::_hx_vtable = *(void **)dummy;
	hx::Static(__mClass) = new hx::Class_obj();
	__mClass->mName = HX_HCSTRING("src.App","\xf7","\x6d","\x18","\x62");
	__mClass->mSuper = &super::__SGetClass();
	__mClass->mConstructEmpty = &__CreateEmpty;
	__mClass->mConstructArgs = &__Create;
	__mClass->mGetStaticField = &hx::Class_obj::GetNoStaticField;
	__mClass->mSetStaticField = &hx::Class_obj::SetNoStaticField;
	__mClass->mMarkFunc = App_obj_sMarkStatics;
	__mClass->mStatics = hx::Class_obj::dupFunctions(0 /* sStaticFields */);
	__mClass->mMembers = hx::Class_obj::dupFunctions(App_obj_sMemberFields);
	__mClass->mCanCast = hx::TCanCast< App_obj >;
#ifdef HXCPP_VISIT_ALLOCS
	__mClass->mVisitFunc = App_obj_sVisitStatics;
#endif
#ifdef HXCPP_SCRIPTABLE
	__mClass->mMemberStorageInfo = App_obj_sMemberStorageInfo;
#endif
#ifdef HXCPP_SCRIPTABLE
	__mClass->mStaticStorageInfo = App_obj_sStaticStorageInfo;
#endif
	hx::_hx_RegisterClass(__mClass->mName, __mClass);
}

} // end namespace src
